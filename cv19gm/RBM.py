...
#Flux
		Param("S_f",0,"Flux S_f",dyn=True,is_valid = is_count,
			func = lambda v,p,d : (lambda pop,s0: p_or_q(v,s0,s0/pop) if p == 'Flux' else p_or_q(v,s0))(d['population'],d['S_0']),
		...
		Param("E_f",0,"Flux E_f",dyn=True,is_valid = is_count,
			func = lambda v,p,d : (lambda pop,e0: p_or_q(v,e0,e0/pop) if p == 'Flux' else p_or_q(v,e0))(d['population'],d['E_0']),
		...
		Param("I_f",0,"Flux I_f",dyn=True,is_valid = is_count,
			func = lambda v,p,d : (lambda pop,i0: p_or_q(v,i0,i0/pop) if p == 'Flux' else p_or_q(v,i0))(d['population'],d['I_0']),
		...
		Param("R_f",0,"Flux R_f",dyn=True,is_valid = is_count,
			func = lambda v,p,d : (lambda pop,r0: p_or_q(v,r0,r0/pop) if p == 'Flux' else p_or_q(v,r0))(d['population'],d['R_0']),
...
#Rates
		...
		Param("tE_I",is_valid = is_count,desc = "Avg. days for symptom incubation"),
		Param("tI_R",is_valid = is_count,desc = "Avg. days for infection"),
	]
...
class SEIRHVD(SEIR):
	...
	STATES = ["S","E","I","R","H","V","D"]
	STATES_NAMES = ["Susceptible","Exposed","Infected","Recovered","Hospitalized","Vaccinated","Dead"]
